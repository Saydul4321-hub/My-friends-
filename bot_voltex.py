...
CONFIG_BOT_TOKEN = "8456361739:AAHh4Nsr9ZG4NUOFOZeJcWF1lPPr2MBUafA"
CONFIG_VOLTEX_API_KEY = "MHFB7MF2GNA"
CONFIG_VOLTEX_BASE_URL = "https://api.2oo9.cloud/MXS47FLFX0U/tnevs/@public/api"
CONFIG_ADMIN_ID = 8361313803
CONFIG_OTP_GROUP_ID = -1003078178967
...
ADMIN_ID = int(os.getenv("ADMIN_ID", str(CONFIG_ADMIN_ID or "8346777366")))
ADMINS = [ADMIN_ID]

OTP_GROUP_ID = int(os.getenv("OTP_GROUP_ID", str(CONFIG_OTP_GROUP_ID or "-1003941189680")))
...
def load_settings():
    ...
    default = {
        "active_panel": "voltex",
        "voltex_api_key": os.getenv("VOLTEX_API_KEY", "").strip() or CONFIG_VOLTEX_API_KEY.strip(),
        "voltex_base_url": os.getenv("VOLTEX_BASE_URL", CONFIG_VOLTEX_BASE_URL),
        "panel_url": f"https://t.me/Voltex_Number_bot?start={ADMIN_ID}",
...
def clean_base_url(url):
    """Normalize panel base URL.
    Keep full path for panels that embed keys in the path
    (e.g. .../@public/api). Only strip a trailing /v1 so
    get_api_urls can safely append /v1/... endpoints.
    """
    url = str(url).strip().rstrip('/')
    # Strip only a trailing /v1 (not /api) so path-based panels stay intact.
    url = re.sub(r'/v1$', '', url)
    return url.rstrip('/')

def get_api_credentials():
    settings = load_settings()
    env_api_key = os.getenv("VOLTEX_API_KEY", "").strip() or CONFIG_VOLTEX_API_KEY.strip()
    configured_api_key = str(settings.get("voltex_api_key", "") or settings.get("voltex_api_key", "")).strip()
    base = settings.get("voltex_base_url") or settings.get("zenex_base_url") or CONFIG_VOLTEX_BASE_URL
    return (env_api_key or configured_api_key,
            clean_base_url(base))

def get_api_urls(base_url):
    # Voltex public API (docs: voltxsms.com)
    # Base already ends with /@public/api
    base = str(base_url).strip().rstrip('/')
    return {
        "getnum": f"{base}/getnum",
        "liveaccess": f"{base}/liveaccess",
        "otp": f"{base}/success-otp",
        "console": f"{base}/console",
    }

def get_api_headers(api_key):
    # Voltex docs: authenticate with the mauthapi header
    return {
        "mauthapi": api_key,
...
def extract_active_ranges(payload):
    """Read active ranges from all supported API response shapes (Voltex + legacy)."""
    if isinstance(payload, list):
        return payload
    if not isinstance(payload, dict):
        return None

    # Voltex liveaccess: { meta, data: { services: [ { sid, ranges: [...] } ] } }
    ...
    if services is not None:
        ...
        for svc in services:
            if not isinstance(svc, dict):
                continue
            sid = svc.get("sid") or svc.get("service") or svc.get("app") or "Unknown"
            for rng in (svc.get("ranges") or []):
                if isinstance(rng, dict):
                    item = dict(rng)
                    item.setdefault("service", sid)
                    grouped.append(item)
                else:
                    grouped.append({"service": sid, "range": rng})
        return grouped
    ...
    for service, values in payload.items():
        if service in ("meta", "message", "rid", "cached") or not isinstance(values, list):
            continue
        for value in values:
...
def bot_settings_keyboard():
    keyboard = [
        ...
        [make_reply_btn("SET OTP CHAT ID",    emoji_id="5231200819986047254", style="primary"),
         make_reply_btn("VOLTEX CONFIG",       emoji_id="5350396951407895212", style="primary")],
        [make_reply_btn("BACK TO ADMIN",      emoji_id="5267490665117275176", style="primary")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)

def voltex_config_keyboard():
    keyboard = [
        ...
        [make_reply_btn("SET ALLOWED SERVICES", emoji_id="5341715473882955310", style="primary"),
         make_reply_btn("VIEW VOLTEX CONFIG",    emoji_id="5231200819986047254", style="primary")],
...
async def show_app_selection(update, context):
    ...
    # Build combined keyboard: Voltex apps + manual services
    async def _build_full_keyboard(top):
        voltex_rows = build_app_buttons_from_cache(top)
        manual_rows = build_manual_service_buttons()
        all_rows = voltex_rows + manual_rows
        if not all_rows:
            all_rows = []
    ...
    if err:
        ...
        if manual_rows:
            ...
            await status.edit_text(
                f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n━━━━━━━━━━━━━━━━━━━━━\n<blockquote>⚠️ Voltex unavailable — showing manual services</blockquote>',
                parse_mode="HTML",
        ...
        return
    if not top:
        # Show only manual services if Voltex fails
        manual_rows = build_manual_service_buttons()
        if manual_rows:
            manual_rows.append([InlineKeyboardButton("⚙️ CUSTOM RANGE", callback_data="custom_range",
                                                       api_kwargs={"style": "primary"})])
            await status.edit_text(
                f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n━━━━━━━━━━━━━━━━━━━━━\n<blockquote>⚠️ Voltex ranges unavailable — showing manual services</blockquote>',
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(manual_rows))
        else:
            await status.edit_text(
...
# ==================== AUTO OTP MONITOR SECTION (VOLTEX) ====================
            ...
            if otps:
                ...
                for otp in otps:
                    ...
                    if (num in active_numbers and
                        ...
                        panel_url = settings.get("panel_url", f"https://t.me/Voltex_Number_bot?start={ADMIN_ID}")
...
async def fetch_number_async(range_str):
    """Allocate a number from Voltex.
    Docs: POST /getnum with body {"rid": "<range digits without trailing XXX>"}.
    """
    range_str = str(range_str or "").strip().upper()
    if not range_str:
        return None

    # Voltex rid = range WITHOUT trailing X's (e.g. 8801 from 8801XXX),
    # or a numeric search-picker range_id.
    rid = re.sub(r"[Xx]+$", "", range_str).strip()
    if not rid:
        rid = range_str
    ...
    try:
        ...
        data = r.json()
        # Voltex envelope: meta.code == 200, number fields under data
        meta = data.get("meta") if isinstance(data, dict) else None
        if isinstance(meta, dict) and meta.get("code") not in (200, "200", None):
            # out of stock etc. (e.g. 2946 not_found)
            return None
        ...
        if num:
            ...
            # Don't treat allocation "message" ("number allocated") as SMS body
            if sms and str(sms).lower() in ("number allocated", "ok", "success"):
                sms = None
                otp = None
            return {
                "number": num,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "BACK TO ADMIN":
        ...
        context.user_data["voltex_config_mode"] = None
    ...
    if text == "VOLTEX CONFIG" and context.user_data.get("bot_settings_mode") == "main" and is_admin(uid):
        context.user_data["voltex_config_mode"] = "main"
        await update.message.reply_text("📡 <b>VOLTEX PANEL CONFIG</b>", parse_mode="HTML", reply_markup=voltex_config_keyboard())
        return
    ...
    # ==================== VOLTEX CONFIG HANDLERS ====================
    if text == "SET API KEY" and context.user_data.get("voltex_config_mode") == "main" and is_admin(uid):
        context.user_data["voltex_config_mode"] = "set_api_key"
        s = load_settings()
        cur = s.get("voltex_api_key", "Not set")
        await update.message.reply_text(
            f"🔑 <b>SET VOLTEX API KEY</b>\n\n<blockquote>Current: <code>{cur[:20]}...</code></blockquote>\n\nSend new API key:",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("voltex_config_mode") == "set_api_key" and is_admin(uid):
        s = load_settings()
        s["voltex_api_key"] = text.strip()
        save_settings(s)
        await update.message.reply_text(
            f"✅ <b>VOLTEX API KEY UPDATED!</b>\n<blockquote><code>{html.escape(text.strip()[:20])}...</code></blockquote>",
            parse_mode="HTML", reply_markup=voltex_config_keyboard())
        context.user_data["voltex_config_mode"] = "main"
        return

    if text == "SET BASE URL" and context.user_data.get("voltex_config_mode") == "main" and is_admin(uid):
        context.user_data["voltex_config_mode"] = "set_base_url"
        s = load_settings()
        cur = s.get("voltex_base_url", "Not set")
        await update.message.reply_text(
            f"🌐 <b>SET VOLTEX BASE URL</b>\n\n<blockquote>Current: <code>{cur}</code></blockquote>\n\nSend new base URL (e.g. https://api.2oo9.cloud/MXS47FLFX0U/tnevs/@public/api):",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("voltex_config_mode") == "set_base_url" and is_admin(uid):
        s = load_settings()
        s["voltex_base_url"] = text.strip().rstrip("/")
        save_settings(s)
        await update.message.reply_text(
            f"✅ <b>BASE URL UPDATED!</b>\n<blockquote><code>{html.escape(s['voltex_base_url'])}</code></blockquote>",
            parse_mode="HTML", reply_markup=voltex_config_keyboard())
        context.user_data["voltex_config_mode"] = "main"
        return

    if text == "SET ALLOWED SERVICES" and context.user_data.get("voltex_config_mode") == "main" and is_admin(uid):
        context.user_data["voltex_config_mode"] = "set_services"
        s = load_settings()
        cur = ", ".join(s.get("allowed_services", []))
        await update.message.reply_text(
            f"📋 <b>SET ALLOWED VOLTEX SERVICES</b>\n\n<blockquote>Current:\n<code>{cur}</code></blockquote>\n\n"
            f"Send comma-separated list:\n<i>e.g. Instagram,Facebook,WhatsApp,TikTok</i>",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("voltex_config_mode") == "set_services" and is_admin(uid):
    ...
    if text == "VIEW VOLTEX CONFIG" and context.user_data.get("voltex_config_mode") == "main" and is_admin(uid):
        s = load_settings()
        api_key = s.get("voltex_api_key", "N/A")
        base_url = s.get("voltex_base_url", "N/A")
        ...
        msg = (
            f"📡 <b>VOLTEX PANEL CONFIGURATION</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        await update.message.reply_text(msg, parse_mode="HTML", reply_markup=voltex_config_keyboard())
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("back_apps", "back_services"):
        top = context.user_data.get("top_ranges_by_app") or (_ranges_cache.get("data") or {})
        manual_rows = build_manual_service_buttons()
        voltex_rows = build_app_buttons_from_cache(top) if top else []
        all_rows = voltex_rows + manual_rows
        all_rows.append([InlineKeyboardButton("\u2699\ufe0f CUSTOM RANGE", callback_data="custom_range",
                                               api_kwargs={"style": "primary"})])
        msg = f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501'
